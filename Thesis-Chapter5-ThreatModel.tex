...
\threattablebegin{Repudiation}{repudiation}{Repudiation}
  ...
  & The escrow agent could require some identifying information before unlocking, such as the LEA's public key digitally
  ...
  & The device is undeniably unlocked, but the vendor claims they did not provide the unlock artifacts, allowing the
    ...
    transparency.
  & The same identifying information used in \ii{R3} would implicate the vendor (assuming the integrity of the vendor's
    signing key).
\threattableend
...
\threattablebegin{Information Disclosure}{disclosure}{Information \\ Disclosed}
  ...
  & The attacker is able to unlock the target device alone. Must be combined with a spoofed DID to be of use to other
    devices (see \ii{S2}). The could record the values to replay them after the phone has been returned.
  ...
  & This is the big prize, the largest source of centralized risk. With the database, attackers could directly
    compromise (or more likely, sell access to) any mobile device using this system in their possession, after
    disassembly, and after the time vault.
  & \ii{R3} defines a mitigation that requires use of a vendor private key to create a digital signature as part of the
    ...
    However, if the database is compromised, that key probably is too. Savage argues that \ac{HSM}s, though imperfect,
...
\threattablebegin{Elevation of Privilege}{privilege}{Privilege Gained}
  E1 & Foreign intelligence or organized crime recruits an insider
...
How much risk does \ldawmsr introduce? The answer varies based on how many mitigations are implemented. If the device is
required to be unlocked on LEA premises and destructive disassembly is chosen, several threats are significantly
reduced. The irreducible risks are those posed by the DID-DSK database and the Secure Enclave itself.
